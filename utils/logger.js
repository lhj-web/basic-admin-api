...
const renderModule = (message) => {
    if (typeof message === 'string' && message.startsWith('[') && message.endsWith(']')) {
        return chalk_1.default.green.underline(message.substring(1, message.length - 1));
    }
    else
        return message;
};
...
const renderLog = (method, levelLabel, messageColor) => {
    return (message, ...args) => {
        if (method === 'dubug')
            return console.debug(chalk_1.default.greenBright('[EDU] - '), renderTime(), levelLabel, renderModule(message), ...renderMessage(messageColor, args));
        return console[method](chalk_1.default.greenBright('[EDU] - '), renderTime(), levelLabel, renderModule(message), ...renderMessage(messageColor, args));
    };
};
const createLogger = () => {
    return {
